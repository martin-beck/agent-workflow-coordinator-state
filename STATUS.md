...
| P0 | [AR-0031](tasks/AR-0031.md): Executable upgrade mutation and rollback boundary | codex-awc-ar0031-mutation-20260918d | SQLite backend.backup campaign parity now has a real-authority success and hostile existing-destination failure test. The generated executor records durable success and preserves journal/lock/destination on failure; broad apply and unsupported mutation remain fail-closed. | Independent-review the exact product commit 65d869d, then consider the corresponding Git backup executor only as a separate bounded slice. Do not enable UpgradeEngine.apply, replace/commit, rollback, selector dispatch, or claim full campaign completion. |
