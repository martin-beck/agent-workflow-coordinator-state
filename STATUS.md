...
| --- | --- | --- | --- | --- |
| P0 | [AR-0007](tasks/AR-0007.md): Upgrade engine and rollback | codex-awc-ar0007-next-20260914 | PR #68 is open at a6a95ce for read-only SQLiteAuthorityAdapter parity: owner-only descriptor checks, integrity/foreign-key evidence, strict context binding, non-authorizing rollback reread, and rejected execute. Independent review found 390 tests, Ruff, mypy, format, and diff pass, but full branch coverage is 94&#37;, below the required 95&#37;; merge is held for targeted coverage repair. Dispatch and all mutation paths remain disabled. | Repair PR #68 coverage to &gt;=95&#37; with focused hostile-branch tests, rerun exact-head full gates and post-merge Verify; do not merge or wire mutation below the coverage gate. AR-0008 remains diagnostic-only. |
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | codex-awc-ar0008-next-20260914 | PR #67 exact-head Git adapter correspondence audit remains diagnostic-only. Formal hashes unchanged (TLA 2a1a31f5, CFG e38502a, evidence 035e6c15); implementation_refinement=not-proven; mutation disabled. Read-only Git snapshot covers clean/reachable HEAD and branch identity only; rollback verification deliberately non-authorizing and execute rejects. No scoped production caller or write/rollback trace exists. | Keep AR-0008 diagnostic-only and mutation disabled. Require ScopedBackendAdapter/LockDomainScope caller traces plus Git crash/restore/selector evidence before mapping write or rollback model actions; preserve formal hashes and nonclaims. |
