...
| P0 | [AR-0064](tasks/AR-0064.md): Authority mutation implementation and formal refinement | Unclaimed | Concrete authority adapters intentionally reject commit/apply today; this child supplies the missing implementation and formal refinement without weakening fail-closed behavior. | Audit runtime bootstrap dispatch admission and the remaining implementation-refinement gap; add only exact non-authorizing identity evidence, then keep Dispatch, mutation, rollback, and release disabled until correspondence is proven. |
