...
| P0 | [AR-0031](tasks/AR-0031.md): Executable upgrade mutation and rollback boundary | codex-awc-ar0031-mutation-20260918d | PR #735 adds bounded, machine-checked formal correspondence scaffolding for SQLite snapshot/identity reread, read-close uncertainty, and permanent ambiguous fencing. It preserves the rejection-only mutation gate. | Correct the SQLite correspondence map: ambiguous-fence must use the existing UpgradeRecovery state-preserving relation, not an invented Acquire action. Add a machine-check that all named model actions exist and rejected operations preserve safe_mode/ambiguous state. Keep all mutation paths fail-closed. |
