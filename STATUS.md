...
| P0 | [AR-0031](tasks/AR-0031.md): Executable upgrade mutation and rollback boundary | codex-awc-ar0031-mutation-20260918d | PR #726 merged as ae2a733 with SQLite real-authority backup campaign evidence. Git still has bound backup primitives but no generated backend.backup executor; broad apply and unsupported mutation remain fail-closed. | Implement the bounded Git generated backend.backup executor/parity from exact main ae2a733 in ar0031-git-backup, with durable identity/journal checks and hostile failure tests. Keep UpgradeEngine.apply, replace/commit/rollback, and selector dispatch disabled. |
