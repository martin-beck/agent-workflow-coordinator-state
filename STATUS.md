...
| P0 | [AR-0012](tasks/AR-0012.md): Durable upgrade barrier and SQLite write fencing | codex-awc-ar0012-barrier-20260918 | Reopened P0 issue #27 work: prior barrier-admission PR history is stale relative to current main b52fcbc; durable barrier/session and every SQLite write-route proof remain required before mutation. | Inspect PR #615 and current main b52fcbc; rebase only if its exact scope remains valid, otherwise implement the next durable barrier/SQLite fencing slice with process-death and write-rejection tests, then run exact-head hosted gates and independent review. |
