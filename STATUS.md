...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | codex-awc-ar0008-next-20260914 | Diagnostic correspondence audit against current scoped adapter prerequisite: model obligations remain abstract and no formal hashes changed. Missing executable-to-model obligations are: (1) bind every UpgradeEngine snapshot/execute/verify_rollback call to one concrete AcquireCommon-&gt;AcquireControl-&gt;AcquireAuthority scope and reverse release trace; (2) prove each PhaseContext lease identity (project, authority revision, fencing token/owner, durable barrier, state revision) maps to the model fence/owner and is reread immediately before the operation; (3) map adapter faults and lock/recheck failures to no-mutation/ambiguous transitions with all locks released; (4) inventory every Git/SQLite authority writer and prove shared fence/CAS plus stale-owner rejection; (5) prove selector publication and runtime validation occur only after backup/commit evidence and rollback reread. Current scope is uncalled, so implementation_refinement remains not-proven and mutation disabled. | Keep AR-0008 diagnostic-only and mutation disabled. Before any model/evidence update, obtain exact traces/tests for the five obligations: per-phase caller binding, lease/fence reread and CAS, failure/lock-release ambiguity, complete writer-route inventory with multiprocess stale rejection, and selector/runtime rollback ordering. Then independently review and run admitted TLC. |
