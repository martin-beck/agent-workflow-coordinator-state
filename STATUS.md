...
| P0 | [AR-0031](tasks/AR-0031.md): Executable upgrade mutation and rollback boundary | codex-awc-ar0031-mutation-20260918d | PR #726 adds real SQLite backend.backup campaign evidence: metadata-bound authority backup succeeds with durable journal outcome, and existing-destination failure preserves journal, destination, and lock state. Broad apply and unsupported mutation remain fail-closed. | Await independent exact-head review and hosted gates for PR #726 at 65d869d. Do not merge until checks pass. After review, consider Git backup parity as a separate slice; keep UpgradeEngine.apply, replace/commit/rollback, and selector dispatch disabled. |
