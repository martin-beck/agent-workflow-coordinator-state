# Agent Workflow Coordinator live project state

Generated from local Git and GitHub. Do not edit.

- Product remote main: `67c0f8a5df77fe7526ad9984df1b5f727ef9a945`
- Local origin/main: `46fd7f59668722beb251ef9e270c7d9d8f421bb3`
- Primary worktree head: `35ec43f98e524db767a308ff9c5ec0906c7f6574`

## Open pull requests

| PR | Head | Base | Merge | Checks | Title |
| --- | --- | --- | --- | --- | --- |
| #85 | `fix/vendor-formal-runtime-v037@73f855d7b71f` | `main` | DIRTY | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SKIPPED, COMPLETED:SUCCESS | fix(vendor): include complete formal runtime closure |
| #315 | `ar0016-postmerge-repair@067ba1cc18c9` | `main` | DIRTY | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:FAILURE, COMPLETED:SKIPPED | fix(ci): honor release timeout for formal validation |
| #317 | `ar0008-reject-rollback-admission@267efd9eb9ef` | `main` | UNSTABLE | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:FAILURE, COMPLETED:SKIPPED | feat(formal): model rollback admission rejection |
| #320 | `fix/formal-attestation-resource-bounds@847c30248a0d` | `main` | DIRTY | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:CANCELLED, COMPLETED:SKIPPED | fix(formal): align attestation resource bounds with tiers |
| #330 | `ar0007-authorized-contract@da1f7c7b558e` | `main` | DIRTY | - | test(upgrade): bind validated rollback backup identity |
| #336 | `ar0007-tier-evidence-hash@f7211c189999` | `main` | DIRTY | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:FAILURE, COMPLETED:SKIPPED | fix(formal): refresh rollback control store hash |
| #367 | `ar0007-git-create-parent@b31a9160c0ca` | `main` | CLEAN | COMPLETED:SUCCESS, COMPLETED:SUCCESS, COMPLETED:SKIPPED, COMPLETED:SUCCESS | fix: bind Git backup destination parent |
| #368 | `ar0007-git-dest-absence@85cae51f1950` | `main` | UNSTABLE | QUEUED:, IN_PROGRESS: | fix: reject Git restore destination creation |

## Recent workflows

| Run | SHA | Event | Workflow | State |
| --- | --- | --- | --- | --- |
| 35059673886 | `85cae51f1950` | pull_request | Verify | in_progress:- |
| 35059606673 | `b31a9160c0ca` | pull_request | Verify | completed:success |
| 35059520632 | `67c0f8a5df77` | push | Verify | completed:success |
| 35059465022 | `59a745452e09` | pull_request | Verify | completed:success |
| 35059442914 | `e88f0c909893` | pull_request | Verify | completed:cancelled |
| 35059226251 | `51fd25a18238` | push | Verify | completed:success |
| 35059159174 | `84fc54e2d2b7` | pull_request | Verify | completed:success |
| 35058924552 | `b526dd5ab393` | push | Verify | completed:success |
| 35058876274 | `09c90f7f62a2` | pull_request | Verify | completed:success |
| 35058716954 | `95832472f976` | push | Verify | completed:success |
| 35058672190 | `5d2dbba2afeb` | pull_request | Verify | completed:success |
| 35058572118 | `f903771a4263` | pull_request | Verify | completed:success |
