...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Exact merged product head 444f23c6 (PR #110) contains only four test assertions for recovered identity-digest encoding; formal artifacts are unchanged. Existing bounded model obligations cover abstract barrier/session safety and recovery, but do not model the concrete identity_digest encoding or the SQLite reader/caller seam. The new assertions map only to implementation evidence for persistence/canonical identity fields, not to a sound refinement relation. | For the next AR-0007/AR-0012 slice, require a concrete production-caller trace that binds journal/control CAS identity to the abstract operation/fence/revision state, including transaction outcome, process death/reopen, stale retry, and cross-store authority checks. Preserve implementation_refinement=not-proven and mutation disabled until the mapping is executable and independently reviewed. | codex-awc-ar0008-next-20260914 |
