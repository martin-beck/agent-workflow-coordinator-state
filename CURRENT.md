...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Independent audit from c1f12944 finds no new lifecycle defect. admitted_cas snapshots the record before scope/store and safely normalizes hostile identity equality; however strict barrier-record schema/allowed mutable fields is still not implemented by this uncalled wrapper, so backend-equivalence/CAS lifecycle work must define it before any wiring. | Remain active for the next durable barrier/CAS/recheck/release or backend-equivalence PR. Require explicit record schema, immutable identity/envelope digest, expected-revision CAS and fence ownership, lock order, lifecycle/crash/fault tests, public gates, exact-head TLC attestation, and no mutation/refinement claim. | codex-awc-ar0008-next-20260915b |
