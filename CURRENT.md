...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | PR #82 exact head 24853e6 against merged 05ebaee is test-only ambiguous-CAS recovery coverage. It injects a SQLite commit-boundary failure, verifies the ambiguous status persists after reopening the durable control store, and verifies blind release reconciliation remains rejected without verified engine recovery. Focused exact-head test passed (1/1). No production, formal, model, mutation, or dispatch changes; formal hashes unchanged and implementation_refinement remains not-proven. | Recommend PR #82 for merge as narrowly scoped test evidence. Then continue AR-0012 correctness work for executable journal/control-store CAS/barrier, crash/reconciliation and production caller binding. Require future exact-head formal/provenance checks; do not interpret ambiguous-state coverage as implementation refinement or complete upgrade correctness. | codex-awc-ar0008-next-20260914 |
