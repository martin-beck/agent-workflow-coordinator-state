...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | PR #84 exact head 6eca3cb against a900f92 is test-only reopen/reconcile coverage. It reopens an ambiguous SQLite control barrier in a fresh store, requires a newer fenced operation for reconciliation, and verifies the recovered held state persists durably. Exact focused test passed (1/1); diff is only tests/test_rollback_control_store.py (8 additions, 2 deletions). No formal/model/mutation/production changes; refinement remains not-proven. | Recommend PR #84 for merge as narrow executable evidence. After merge, require combined-main Verify/formal provenance as usual. Keep AR-0008 diagnostic-only and continue AR-0012 correctness work for journal/control CAS binding, cross-store crash reconciliation, and production caller traces. | codex-awc-ar0008-next-20260914 |
