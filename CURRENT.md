...
| --- | --- | --- | --- | --- |
| P0 | [AR-0007](tasks/AR-0007.md): Upgrade engine and rollback | PR #71 at d259a04 exercises a real SQLite BEGIN EXCLUSIVE/CAS transaction and pre-commit process termination. Independent exact-head review found the implementation and 397-test suite pass, but the rollback claim lacks an explicit post-termination authority-row assertion (body == clean); merge is held pending that evidence. Mutation/dispatch/upgrade routes remain disabled. | Amend PR #71 with an explicit fresh-reader assertion of the pre-transaction authority value, rerun focused/full gates and exact-head CI, then review and merge only after the corrected evidence is present. AR-0008 remains diagnostic-only. | codex-awc-ar0007-next-20260914 |
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Exact-head audit at 6a223c9 confirms no transaction-boundary implementation has appeared: SQLiteAuthorityAdapter.execute remains rejection-only and no production adapter binds UpgradeEngine to LockDomainScope. Existing sidecar tests provide only synthetic identity rejection. Acceptance trace schema for next AR-0007 slice: each case records exact product/formal hashes, backend/schema/project binding, WAL mode and synchronous setting, process IDs, lock/barrier/session/fence revisions, SQLite journal/WAL/SHM identities, operation/fault injection point, pre/post authority and projection digests, transaction outcome/exit signal, durable journal/control records, reconciliation decision, and independent reviewer. Required cases: BEGIN IMMEDIATE contention/CAS stale writer; crash before commit; crash after commit before projection/journal; WAL/SHM removal/replacement/checkpoint; online backup/restore with control DB exclusion; rollback validation failure and ambiguous safe mode; fresh newer-fence recovery. Existing TLA maps abstractly to RequestWrite/AcceptWrite/FinishWrite, CasBounded, StaleCASRejected, WriteFence, ObserveAuthority, RecheckEvidence, MarkAmbiguous; no trace relation yet. | Keep AR-0008 diagnostic-only and mutation disabled. Await a concrete transaction-boundary/WAL harness checkpoint from AR-0007 with the required trace schema; only then decide whether a companion model or bounded model extension is justified. Preserve implementation_refinement=not-proven and formal hashes; no TLC warranted for this test-only/read-only head. | codex-awc-ar0008-next-20260914 |
