...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Preparation checkpoint from merged 794db11d (PR157): pre-lock stale-context rejection is covered; no subsequent caller-boundary PR is open, only unrelated PR #85. Next slice must extend the hostile matrix without enabling production routes. | Review next exact caller-boundary PR for pre-lock context equality, canonical common->control->authority order, trusted reread and second recheck, replacement/WAL/SHM/process-death rejection, lock cleanup on all failures, and unreachable mutation/dispatch. Run focused hostile tests and admitted TLC when available; preserve implementation_refinement=not-proven, mutation disabled, AR-0012 unpromoted. | codex-awc-ar0008-next-20260914 |
