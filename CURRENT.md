...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Formal/trace acceptance checklist prepared at exact product 58cb897 for next SQLite transaction/WAL fault slice. Transaction gate: use real SQLiteBackend with bound mutation_scope; record BEGIN IMMEDIATE, conditional revision CAS, commit/rollback outcome, busy/competing writer, crash before commit and after commit-before-projection, and exact authority/projection/event revisions; map to HandoffctlStorage transaction invariants plus UpgradeBarrier RequestWrite/AcceptWrite/FinishWrite, CasBounded, StaleCASRejected, WriteFence. WAL/SHM gate: prove WAL mode, synchronous FULL, same-host filesystem admission, online backup/checkpoint including -wal/-shm, reader/writer behavior, WAL/SHM removal/replacement and descriptor device/inode swaps, integrity/FK and authority revision reread; map to ObserveAuthority/RecheckEvidence/IdentityStable while keeping VFS outside current model. Recovery gate: SIGKILL at each transaction/backup/reopen boundary, durable journal/control reconciliation, no duplicate restore, lock release, ambiguous safe mode, and fresh fenced session only after reconciliation. Evidence must include exact head, process/DB setup, fault point, before/after revisions/digests, exit classification, and independent review. None of this is current refinement proof. | Keep AR-0008 diagnostic-only and mutation disabled. Require the real transaction/WAL/SHM harness and exact traces above before proposing a companion model or formal hash update; preserve implementation_refinement=not-proven and no TLC. | codex-awc-ar0008-next-20260914 |
