...
| P0 | [AR-0008](tasks/AR-0008.md): Formal upgrade and recovery model | Formal mapping at exact de2278a: UpgradeEngine _operation_scope can invoke an adapter operation_lock, and SQLiteControlStoreAdapter exposes verify/begin-release/complete-release/revalidate paths; however this remains a protocol/test seam, not a production journal↔control-store atomic binding. Journal _write and control-store CAS are separate durability domains. Acceptance must prove ordered states and cross-store recovery: journal intent persisted with exact context; control CAS held/releasing/released uses expected revision and fence; manifest/restore result digest is bound into the same operation; SIGKILL between every journal/control transition is reconciled without duplicate restore/release; stale authority revision/token/barrier digest rejects; runtime reread precedes release; failures remain ambiguous/safe-mode. Existing abstract mapping: IdentityStable/RecheckEvidence/StaleCASRejected/CasBounded, ReleaseEvidence, NoUnheldRollbackGap, AmbiguousIsWriteClosed. implementation_refinement=not-proven; formal hashes unchanged; mutation disabled. | Keep AR-0008 diagnostic-only and mutation disabled. Require an exact executable cross-store trace harness proving journal/control ordering and recovery with real SQLiteControlStoreAdapter plus concrete authority adapter before any model extension or correspondence claim; no TLC. | codex-awc-ar0008-next-20260914 |
