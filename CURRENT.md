...
| Priority | Task | Summary | Next action | Owner |
| --- | --- | --- | --- | --- |
| P0 | [AR-0012](tasks/AR-0012.md): Durable upgrade barrier and SQLite write fencing | Durably fence upgrade admission and every SQLite authority mutation under one project barrier. | Implement initialized production-compatible authority schema/factory and real four-route lifecycle tests; use them to cover sqlite_storage and strict factory branches, then rerun hosted pr-fast. Do not merge until route/failure/concurrency/formal evidence and independent review are complete. | - |
